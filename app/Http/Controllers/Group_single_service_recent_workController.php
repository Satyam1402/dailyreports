...
class Group_single_service_recent_workController extends Controller
{
    public function index($group_item_id = null,$group_category_id=null)
    {
        ...
        // die;
        $data = Group_single_service_recent_work::with(['groupserviceitem'])->get();

        return view('service.group_single_service_recent_work.group_single_service_recent_work',compact('data','group_item_id','category_id','group_category_id')); 
    }
    ...
    public function getData(Request $request)
    {
        if ($request->ajax()) {
            ...
            // ];  
            $columns = [
                'id','group_service_item_id','recent_work_video_thumbnail','recent_work_video','display_order','status'
            ];
            ...
            // $data = Group_single_service_recent_work::select('*')->with(['groupserviceitem'])->orderBy($sortColumn, $sortDirection);

            $query = Group_single_service_recent_work::select('*')->with(['groupserviceitem']);
            ...
            return DataTables::of($data)
                ...
                // })
                ->addColumn('recent_work_video_thumbnail', function ($row) {
                    $imgPath = $row->recent_work_video_thumbnail ?? '';
                ...
                // ->editColumn('recent_work_video', function ($row) {
                //     $imgPath = $row->recent_work_video ?? '';
                //     $baseUrl = env('AWS_URL'); // Get the base URL from the .env file
                //     $imgUrl = $baseUrl . '/' . $imgPath; // Concatenate the base URL with the stored path
                //     return '<a href="'. $imgUrl. '" target="_blank">Click</a>';
                // })
                ...
                ->addColumn('action', function ($row) use ($group_category_id){
                    ...
                    return '
                        <div class="d-flex">
                            <a href="' . route('group-single-service-recent-work.show',['id' => $row->id, 'group_item_id' => request('group_item_id'),'group_category_id' => request('group_category_id')]) . '" class="mb-1 btn btn-primary btn-sm mr-2">
                                <i class="fas fa-edit"></i>
                            </a>
                            '.$previewButton.'
                            <a href="#" class="mb-1 btn btn-danger btn-sm" onclick="confirmDelete(\'' . route('group-single-service-recent-work.destroy', ['id' => $row->id, 'group_item_id' => request('group_item_id'), 'group_category_id' => request('group_category_id')]) . '\');">
                                <i class="fas fa-trash"></i>
                            </a>
                        </div>
                    ';
                })
                ->rawColumns(['group_service_item_title','recent_work_video_thumbnail','recent_work_video','status','action'])
                ->make(true);
        }
    }
    ...
    public function add($group_item_id = null,$group_category_id=null){
       ...
       return view('service.group_single_service_recent_work.add_group_single_service_recent_work', compact('groupserviceitem','group_item_id','group_category_id'));
    ...
    public function store(Request $request)
    {
        ...
        // $video = upload_file_to_s3($request, 'recent_work_video', 'service-recent-work-video');
        $image = upload_file_to_s3($request, 'recent_work_video_thumbnail', 'service-recent-work-video-thumbnail');
        $userId=Auth::user()->id;

        $data = new Group_single_service_recent_work;
        ...
        $data->recent_work_video_thumbnail = $image ?? '';
        // $data->recent_work_video = $video ?? '';
        $data->recent_work_video = $request->recent_work_video ?? '';
        $data->display_order = $request->display_order ?? 0;
        $data->user_id = $userId;
        $data->status=$request->status ?? 0;
        $data->save();

        return redirect()->route('group-single-service-recent-work.index',['group_item_id'=>$group_item_id,'group_category_id'=>$group_category_id]);
        // return redirect('group/service/group_single_service_recent_work');
    ...
    public function show(Request $request,$id,$group_item_id = null,$group_category_id=null)
    {
        ...
        $data = Group_single_service_recent_work::with(['groupserviceitem'])->get();
            ...
            // $homeservicecategory = Service_category::query()->get();
            $groupserviceitem = Group_service_item::query()->get();
            $data = Group_single_service_recent_work::find($id);
            $selectedItem = $data->group_service_item_id;
            // print_r($selectedCategory->toArray());
            // print_r($selectedItem);
            // die;
            return view('service.group_single_service_recent_work.edit_group_single_service_recent_work',compact('data','groupserviceitem', 'selectedItem','group_item_id','group_category_id'));
    ...
    public function update(Request $request)
    {
        ...
        $update = [
            ...
            'recent_work_video_thumbnail' => $image ?? $data->recent_work_video_thumbnail ?? '',
            // 'recent_work_video' => $video ?? $data->recent_work_video ?? '',
            'recent_work_video' => $request->recent_work_video ?? '',
